...
const ColumnDevelopment = require("../models/ColumnDevelopment");

// Get all active Development columns
router.get("/", async (req, res) => {
    try {
        const columns = await ColumnDevelopment.find({ status: { $ne: 'deactive' } }).sort({ order: 1 });
        res.json(columns);
    } catch (err) {
...
router.post("/", async (req, res) => {
    try {
        ...
        const lastColumn = await ColumnDevelopment.findOne().sort({ order: -1 });
        ...
        const column = new ColumnDevelopment({
            column_heading,
            name,
...
router.put("/:name/options", async (req, res) => {
    try {
        ...
        const result = await ColumnDevelopment.updateOne(
...
router.put("/reorder/update", async (req, res) => {
    try {
        ...
        const updatePromises = columnOrders.map(item =>
            ColumnDevelopment.updateOne({ name: item.name }, { $set: { order: item.order } })
        );
...
router.put("/:name", async (req, res) => {
    try {
        ...
        await ColumnDevelopment.updateOne(
            { name: req.params.name },
...
router.patch("/:name/access", async (req, res) => {
    try {
        ...
        const updated = await ColumnDevelopment.findOneAndUpdate(
            { name: req.params.name },
            updateData,
            { new: true }
        );
