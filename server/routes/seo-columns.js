...
const SeoColumn = require("../models/SeoColumn");
// Get all active columns
router.get("/", async (req, res) => {
    try {
        const columns = await SeoColumn.find({ status: { $ne: 'deactive' } }).sort({ order: 1 });
        res.json(columns);
    } catch (err) {
...
router.post("/", async (req, res) => {
    try {
        ...
        const lastColumn = await SeoColumn.findOne().sort({ order: -1 });
...
router.put("/:name/options", async (req, res) => {
    try {
        ...
        const result = await SeoColumn.updateOne(
...
router.put("/reorder/update", async (req, res) => {
    try {
        ...
        const updatePromises = columnOrders.map(item =>
            SeoColumn.updateOne({ name: item.name }, { $set: { order: item.order } })
        );
...
router.patch("/:name/access", async (req, res) => {
    try {
        ...
        const updated = await SeoColumn.findOneAndUpdate(
            { name: req.params.name },
            updateData,
            { new: true }
        );
