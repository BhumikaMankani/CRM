...
const ColumnSeo = require("../models/ColumnSeo");

// Get all active SEO columns
router.get("/", async (req, res) => {
    try {
        const columns = await ColumnSeo.find({ status: { $ne: 'deactive' } }).sort({ order: 1 });
        res.json(columns);
    } catch (err) {
...
router.post("/", async (req, res) => {
    try {
        ...
        const lastColumn = await ColumnSeo.findOne().sort({ order: -1 });
...
router.put("/:name/options", async (req, res) => {
    try {
        ...
        const result = await ColumnSeo.updateOne(
...
router.put("/reorder/update", async (req, res) => {
    try {
        ...
        const updatePromises = columnOrders.map(item =>
            ColumnSeo.updateOne({ name: item.name }, { $set: { order: item.order } })
        );
...
router.patch("/:name/access", async (req, res) => {
    try {
        ...
        const updated = await ColumnSeo.findOneAndUpdate(
            { name: req.params.name },
            updateData,
            { new: true }
        );
